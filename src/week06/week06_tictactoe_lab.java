...
public class week06_tictactoe_lab {
	...
	// This Lab requires you to program a simple game of Tic Tac Toe
		...
		while (winner.equals("") && allSquares <= 9) {
			System.out.println("It's "+player +"'s turn: enter a position number to place an "+ player + " inside." );
			String readInput = readIn.nextLine();
			squareNumber = Integer.valueOf(readInput);
			
			if (squareNumber > 0 && squareNumber <= 9) {
				if (newGame.ifValidSetSquare(squareNumber, readInput, player)){
						if (player.equals("X")) {
							player = "O";
						} else {
							player = "X";
						...
						System.out.println("Square has already been filled. Try again.");
					}
				}
			}
		System.out.println("-------------------------------------");
